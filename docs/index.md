# aiPal Documentation Index

This page provides an overview of all documentation for aiPal. Start here to find guides for setup, usage, deployment, and development.

## Getting Started
- [Getting Started](./getting-started.md) — Quick local installation and first steps.
- [New User Guide](../README.md#new-user-guide) — Overview of features and understanding aiPal.
- [Knowledge Base (RAG)](./knowledge-base.md) — Upload documents and query your personal knowledge base.

## Setup & Integrations
- [Deploy to VPS](./deploy-vps.md) — Production deployment with HTTPS.
- [Telegram Setup](./telegram-setup.md)
- [WhatsApp Setup](./whatsapp-setup.md)
- [Google OAuth & Gmail](./google-oauth-setup.md)
- [Microsoft OAuth](./microsoft-oauth-setup.md)
- [Browser Extension](./browser-extension.md)
- [Daily Briefing](./daily-briefing.md)
- [Personal CRM](./personal-crm.md)
- [Knowledge Base (RAG)](./knowledge-base.md) — RAG setup and usage details.

## Operations & Support
- [Monitoring](./monitoring.md)
- [Backup & Restore](./backup-restore.md)
- [Troubleshooting](./troubleshooting.md)
- [FAQ](./faq.md)
- [User Permissions](./user-permissions.md)

## Architecture & Development
- [Architecture Overview](./architecture.md) — High-level design.
- [API Overview](./api-overview.md) — REST API usage and endpoints.
- See also the detailed [ARCHITECTURE.md](../ARCHITECTURE.md) in the project root.
- [Contribution Guidelines](./contribution-guidelines.md)
- Main [CONTRIBUTING.md](../CONTRIBUTING.md) and [CHANGELOG.md](../CHANGELOG.md)

## Additional Resources
- [Security](../SECURITY.md) — Security policy and considerations.
- [Features](../FEATURES.md) — Detailed feature list.
- [Root README](../README.md)

For the latest updates, check the [Changelog](../CHANGELOG.md).

If you find a documentation gap, please [open an issue](https://github.com/Samireltabal/aiPal/issues) or contribute via PR.
